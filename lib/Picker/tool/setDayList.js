...
function createDayArr(starTime, endTime, pickupDay, returnDay, yesterdayClick) {
    ...
    if (yesterdayClick) {
        if (starMonth != 0) {
            var _currDate = new Date(starYear, starMonth - 1, 1);
            var _currDayCount = getMonthTotalDay(_currDate);
            var _dayList = getMonthDayArray(_currDate, _currDayCount, pickupDay, returnDay, yesterdayClick);
            dateArray.unshift({
                year: _currDate.getFullYear().toString(),
                month: (_currDate.getMonth() + 1).toString(),
                dayList: _dayList,
                isShow: _isShowMonth(_dayList) //在渲染时时候显示出来，
            });
            // 今年的第一个月
        } else {
            var _currDate2 = new Date(starYear - 1, 11, 1);
            var _currDayCount2 = getMonthTotalDay(_currDate2);
            var _dayList2 = getMonthDayArray(_currDate2, _currDayCount2, pickupDay, returnDay, yesterdayClick);
            dateArray.unshift({
                year: _currDate2.getFullYear().toString(),
                month: (_currDate2.getMonth() + 1).toString(),
                dayList: _dayList2,
                isShow: _isShowMonth(_dayList2) //在渲染时时候显示出来，
            });
        }
    }
