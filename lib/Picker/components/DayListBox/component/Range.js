    ...
    _createClass(Range, [{
        ...
        value: function synchronizationReturnTimeEnd(endData) {
            ...
            if (this.props.type == 'return') {
                this.setState(endData, function () {
                    _this6.props.selectTime(_this6.state.currTime, _this6.props.type);
                });
            }
        }
        ...
        value: function setDefaultTime() {
            ...
            if (targetTime == null) {
                ...
                // 同步顶部SelectedTime
                var synchTargetTimeArr = targetTime.content.split(':');
                var type = this.props.type == 'pickup' ? 'pickupTime' : 'returnTime';
        ...
        key: 'getTimeRange',
        value: function getTimeRange(nextProps) {
            var _ref = nextProps ? nextProps : this.props,
                timeRange = _ref.timeRange,
                yesterdayTimeRange = _ref.yesterdayTimeRange,
                day = _ref.day,
                isYesterday = _ref.isYesterday,
                pickupDay = _ref.pickupDay;
        ...
        key: 'setTimeArr',
        value: function setTimeArr(totalSlideWidth, nextProps) {
            // 接受更新的props或者本身的props达到更新的效果
            var range = this.getTimeRange(nextProps);
            ...
            var step = range.start == 0 ? (range.end - range.start) * 2 : (range.end - range.start) * 2 + 1,
        ...
        key: 'setTimeArrV2',
        value: function setTimeArrV2(tRange, totalSlideWidth) {
            var arr = [];
            var currentTime = this.props.currentTime;

            var timeGap30 = 1800000; // 30分钟
            var countStart = new Date(currentTime.y + '-' + currentTime.month + '-' + currentTime.d + ' ' + tRange.start);
            var countEnd = new Date(currentTime.y + '-' + currentTime.month + '-' + currentTime.d + ' ' + tRange.end);
            var start = {
            ...
            var step = Math.ceil((countEnd - countStart) / timeGap30);
            var hour = start.h;
            var count = 0;
